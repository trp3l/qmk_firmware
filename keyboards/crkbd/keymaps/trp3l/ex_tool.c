...
bool is_auto_repeating = false;
uint16_t auto_repeat_dilay = 0;

boid auto_repeat_matrix_scan(keyrecord_t *record){
	if(is_auto_repeating &&
		 record->event.pressed &&
		 timer_elapsed(auto_repeat_dilay) > auto_repeat_toggle){
			  set_ninputs_i(get_keycode_prev());
				add_chrcount();
				naginata_type();
				auto_repeat_dilay = timer_read();
  }
}
