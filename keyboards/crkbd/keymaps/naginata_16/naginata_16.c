...
//trp3l ng_16
static uint16_t keycomb = 0UL; // 同時押しの状態を示す。32bitの各ビットがキーに対応する。
//trp3l ng_16
...
#define B_S    (1UL<<11)
#define B_D    (1UL<<12)
#define B_F    0b1100001000100100
#define B_G    (1UL<<13)

#define B_H    (1UL<<14)
#define B_J    0b1010000101000100
#define B_K    0b0011010011000000
#define B_L    0b0010110000000110
#define B_SCLN (1UL<<15)

#define B_Z    0b0110001100010000
#define B_X    0b0101001010001000
#define B_C    0b0000010100100011
#define B_V    0b0000100010101001
#define B_B    0b0001100001010010

#define B_N    0b1111100000000000
#define B_M    0b0001111100000000
#define B_COMM 0b0000001111100000
#define B_DOT  0b0000010001111000
#define B_SLSH 0b0000000000011111

#define B_SHFT 0b1100000010000011
//trp3l ng_16
...
const PROGMEM naginata_keymap ngmap[] = {
  ...
  {.key = B_I                      , .kana = "ru"      }, // る
  {.key = B_SLSH                   , .kana = "he"      }, // れ  → へ

  {.key = B_SHFT^B_SLSH            , .kana = "he"      }, // れ  → へ
  {.key = B_A                      , .kana = "ro"      }, // ろ
  ...
  {.key = B_F^B_SLSH               , .kana = "be"      }, // べ
  {.key = B_F^B_SLSH^B_SHFT        , .kana = "be"      }, // べ(冗長)
...
//  {.key = B_V^B_P^B_SHFT           , .kana = "pe"      }, // ぺ(冗長)
  {.key = B_V^B_SLSH                   , .kana = "pe"      }, // ぺ
  {.key = B_V^B_SLSH^B_SHFT            , .kana = "pe"      }, // ぺ(冗長)
 ...
 // {.key = B_Q^B_SHFT^B_P           , .kana = "xyu"     }, // ゅ
  {.key = B_Q^B_SLSH                  , .kana = "xyu"     }, // ゅ
  {.key = B_Q^B_SHFT^B_SLSH           , .kana = "xyu"     }, // ゅ
  ...
  {.key = B_M^B_E^B_SLSH              , .kana = "thu"     }, // てゅ
  {.key = B_M^B_E^B_SLSH^B_SHFT       , .kana = "thu"     }, // てゅ(冗長)
  {.key = B_J^B_E^B_K              , .kana = "dhi"     }, // でぃ
  {.key = B_J^B_E^B_K^B_SHFT       , .kana = "dhi"     }, // でぃ(冗長)
  {.key = B_J^B_E^B_SLSH              , .kana = "dhu"     }, // でゅ
  {.key = B_J^B_E^B_SLSH^B_SHFT       , .kana = "dhu"     }, // でゅ(冗長)
  ...
  {.key = B_V^B_DOT^B_SLSH            , .kana = "fyu"     }, // ふゅ
  {.key = B_V^B_DOT^B_SLSH^B_SHFT     , .kana = "fyu"     }, // ふゅ(冗長)
  ...
  {.key = B_F^B_L^B_SLSH              , .kana = "vuxyu"   }, // ゔゅ
  {.key = B_F^B_L^B_SLSH^B_SHFT       , .kana = "vuxyu"   }, // ゔゅ(冗長)
...
//trp3l
  {.key = B_A^B_Q       , .kana = SS_LCTRL("x")},
  {.key = B_A^B_R       , .kana = SS_LCTRL("c")},
  {.key = B_A^B_S       , .kana = SS_LCTRL("z")},
  {.key = B_A^B_D       , .kana = SS_LCTRL("y")},
  {.key = B_A^B_C       , .kana = SS_LCTRL("a")},
  {.key = B_A^B_V       , .kana = SS_LCTRL("f")},
  {.key = B_A^B_B       , .kana = SS_LCTRL("v")},
//_trp3l
...
//trp3l"
  ...
  {.key = B_D^B_F^B_L       , .kana = SS_TAP(NGLT)SS_TAP(NGLT)SS_TAP(NGLT)SS_TAP(NGLT)SS_TAP(NGLT)},
  ...
  {.key = B_D^B_F^B_DOT     , .kana = SS_TAP(NGRT)SS_TAP(NGRT)SS_TAP(NGRT)SS_TAP(NGRT)SS_TAP(NGRT)},
...
//trp3l 固有名詞モード
  {.key = B_S^B_D^B_I       , .kana = "2190"}, // ↑
  {.key = B_S^B_D^B_K       , .kana = "2191"}, // ↓
  {.key = B_S^B_D^B_J       , .kana = "2192"}, // ←
  {.key = B_S^B_D^B_L       , .kana = "2193"}, // →
//trp3l
#endif
};
...
bool process_naginata(uint16_t keycode, keyrecord_t *record) {
  ...
  if (record->event.pressed) {
    switch (keycode) {
      ...
      case NG_RIGHT:
       if(arrow_pressed && TIMER_DIFF_16(record->event.time, arrow_timer) < 100){
